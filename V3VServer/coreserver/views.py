...
def index(request):
    ...
    return render_to_response('coreserver/v3v_demo_home.html', context)
...
def upload_and_convert_segment(request):
        ...
        template = loader.get_template('coreserver/v3v_demo_home.html')
...
@csrf_exempt
def get_video_duration_count(request):
    data = Segment2D.objects.values('duration').annotate(count=Count('id'))
    data_list = list(data)
    remove_none_element(data_list, 'duration')
    return JsonResponse(data_list, safe=False)
