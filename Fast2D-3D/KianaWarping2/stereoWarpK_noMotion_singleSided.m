function [lefts, rights, disparity] = stereoWarpK_noMotion_singleSided( imgs, imgs_RAW, depths, flows, displevels,R)
...
%% Optimize
    ...
    %fprintf('\tOptimizing...\n\t');
    %tic
    [x, flag] = pcg(A,b,0.5,150,M,M',disparity0(:)); %1-e6
    clear flag
   %toc
...
%tic
    ...
    lefts = imgs_RAW;
    %rights = clip(warpImage_v2(imgs_RAW, warpright, zeros(h,w),R));
    warpright = imresize(warpright,[size(imgs_RAW,1) size(imgs_RAW,2)]);
    
    rights = gather(clip(warpImage_v2(gpuArray(imgs_RAW), gpuArray(warpright), gpuArray(zeros(size(imgs_RAW,1), size(imgs_RAW,2))),R)));
