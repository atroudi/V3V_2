function SS = SimpleInterpolation(RAW,D,Thresh,R,xx,yy,YY)
...
I = imresize(RAW,[size(D,1) size(D,2)]);

%%%Warping

[lefts, rights, disparity] = stereoWarpK_noMotion_singleSided_Simple(I, RAW, D, Thresh, xx, yy, YY);

%%%Creat Side by Side
[vres hres u] = size(lefts);
SS = imresize([lefts(:,Thresh:end,:) rights(:,Thresh:end,:)],[vres 2*hres]);

end

function [lefts, rights, disparity] = stereoWarpK_noMotion_singleSided_Simple( imgs, imgs_RAW, depths, displevels, xx, yy, YY)
...
rights = (clip(warpImage_v2((imgs_RAW), (warpright), xx, yy, YY)));
...
function [warpI2,mask]=warpImage_v2(im,vx,xx,yy,YY)
