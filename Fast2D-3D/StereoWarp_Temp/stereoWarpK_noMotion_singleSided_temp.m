function [lefts, rights, disparity] = stereoWarpK_noMotion_singleSided_temp( imgs, imgs_RAW, depths, Gx, Gy, flows, displevels, R, xx, yy, YY, X, Y, IDX, rowidx, colidx)
...
%     depths(depths>highprc) = highprc;
    ...
    Gt = getVideoGradients_onlymotion( flows, X, Y, IDX, rowidx, colidx );
...
%     rights = zeros([u/R p/R q z]);
    ...
    lefts = zeros(size(imgs_RAW));
    rights = zeros(size(imgs_RAW));
    for i=1:K-1
            lefts(:,:,:,i) = imgs_RAW(:,:,:,i);
            this_warpright = imresize(warpright(:,:,:,i),[size(imgs_RAW,1) size(imgs_RAW,2)]);
            rights(:,:,:,i) = (clip(warpImage_v2((imgs_RAW(:,:,:,i)), this_warpright, R, xx, yy, YY)));
    end   
