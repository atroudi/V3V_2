function CLASS = GetSceneClassification(rootin,root,Cuts,resize_factor)
...
I = double(imresize(imread([rootin num2str(ALLFR(1)) '.png']),resize_factor));
[vres, hres, u] = size(I);
num_pels = vres*hres;
...
for fr = CutStFR,
    ...
    for f = st:endi,
        ...
        clear I
        I = double(imresize(imread([rootin num2str(f) '.png']),resize_factor));
        ...
        imwrite(not(pitchall),[root,'/mask/', num2str(f) '.png']);
