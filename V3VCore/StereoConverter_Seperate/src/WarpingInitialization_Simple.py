...
R: resize factor
"""
def WarpingInitialization_Simple(vres, hres):
    """
    # Make new image (flows) of size vres*hres resized by factor (R) initialized by all zeroes
    flows = cv2.resize(np.zeros((vres, hres), dtype=np.uint8), (0, 0), fx=R, fy=R, interpolation=cv2.INTER_CUBIC)
    ...
    return xx, yy, YY
"""
def getVideoGradients_noMotion(flows):
